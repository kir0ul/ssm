...
A0 = 0.99 * random_rotation(D, theta=np.pi / 20)
# S = (1 + 3 * npr.rand(D))
S = np.arange(1, D + 1)
...
lds = LDS(N, D, emissions="gaussian")
lds.initialize(y)
q_struct_elbos, q_struct = lds.fit(
    y,
    method="bbvi",
...
for n in range(N):
    ...
    plt.plot(
        q_mf_y[:, n] + 4 * n, "--", color=colors[0], label="MF" if n == 0 else None
    )
    plt.plot(
        q_struct_y[:, n] + 4 * n,
        ":",
    ...
    plt.plot(
        ...
        "--",
        color=colors[2],
        label="Laplace-EM" if n == 0 else None,
    )
plt.legend()
plt.xlabel("time")
plt.xlim((0, 200))

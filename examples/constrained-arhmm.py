...
class MixtureOfGaussians(object):
    ...
    def fit(self, data):
        ...
        self.observations._sqrt_Sigmas = np.linalg.cholesky(
            Sigmas + 1e-8 * np.eye(self.D)
        )
...
def _gibbs_sample_discrete_states(arhmm, continuous_states):
    ...
    # Compute potentials
    log_pi0 = arhmm.init_state_distn.log_initial_state_distn(
        continuous_states, input, mask, tag, None
    )
    log_Ps = arhmm.transitions.log_transition_matrices(
        continuous_states, input, mask, tag, None
    )
    log_likes = arhmm.observations.log_likelihoods(
        continuous_states, input, mask, tag, None
    )
...
def _gibbs_sample_continuous_states(arhmm, discrete_states, indicators, mog, beta=1):
    ...
    # Call the forward filter backward sample code
    return lds_sample(
        As, bs, Qi_sqrts, ms, 1e-4 * np.eye(arhmm.D) + np.sqrt(beta) * Ri_sqrts
    )
...
def _gibbs_sample_indicators(continuous_states, mog, beta=1):
    ...
    # Compute potentials
    log_pi0 = mog.prior.log_initial_state_distn(
        continuous_states, input, mask, tag, None
    )
    ...
    log_likes = mog.observations.log_likelihoods(
        continuous_states, input, mask, tag, None
    )
...
def gibbs_sample_constrained_arhmm(
    T, arhmm, mog, num_samples=25, num_burnin=100, burnin_schedule=None
):
    """
    ...
    for itr in trange(num_burnin + num_samples):
        ...
        continuous_states = _gibbs_sample_continuous_states(
            arhmm, discrete_states, indicators, mog, beta=beta
        )
...
ys, masks, z_trues, z_key, neuron_names = load_kato_data(
    include_unnamed=False, signal="dff"
)
os.chdir(tmp)
...
def sample(
    T=3000, num_samples=25, num_burnin=100, schedule=None, filename="samples.mp4"
):

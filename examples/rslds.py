...
def make_nascar_model():
    As = [
        random_rotation(D_latent, np.pi / 24.0),
        random_rotation(D_latent, np.pi / 48.0),
    ]
...
q_elbos_svi, q_svi = rslds_svi.fit(
    y, method="bbvi", variational_posterior="tridiag", initialize=False, num_iters=1000
)
...
plot_most_likely_dynamics(
    rslds_lem, xlim=(-lim[0], lim[0]), ylim=(-lim[1], lim[1]), ax=ax
)

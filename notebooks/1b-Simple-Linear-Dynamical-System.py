...
A0 = 0.99 * random_rotation(state_dim, theta=np.pi / 20)
...
q = plot_dynamics_2d(
    A, bias_vector=b, mins=states.min(axis=0), maxs=states.max(axis=0), color=colors[0]
)
...
plt.yticks(
    np.arange(state_dim) * lim, ["$x_{}$".format(d + 1) for d in range(state_dim)]
)
plt.xticks([])
...
plt.yticks(
    -np.arange(obs_dim) * lim, ["$y_{{ {} }}$".format(n + 1) for n in range(obs_dim)]
)
plt.xlabel("time")
plt.xlim(0, time_bins)
...
for d in range(state_dim):
    ...
    plt.plot(
        ...
        "--",
        color=colors[0],
        label="Estimated States" if d == 0 else None,
    )
plt.ylabel("$x$")
plt.xlabel("time")
...
for n in range(obs_dim):
    ...
    plt.plot(
        ...
        "--",
        color=colors[0],
        label="Smoothed" if n == 0 else None,
    )
plt.legend()
plt.xlabel("time")

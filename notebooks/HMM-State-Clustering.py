...
true_hmm.observations.mus[-1] = true_hmm.observations.mus[-2] + 1e-3 * npr.randn(
    obs_dim
)
...
for k in range(num_states):
    plt.contour(
        XX, YY, np.exp(lls[:, k]).reshape(XX.shape), cmap=white_to_color_cmap(colors[k])
    )
    plt.plot(
        data[true_states == k, 0],
        data[true_states == k, 1],
        "o",
        mfc=colors[k],
        mec="none",
        ms=4,
...
def merge_two_states(hmm, s1, s2, datas, observations="gaussian"):
    ...
    expectations = [
        hmm.expected_states(data, input, mask, tag)
    ...
    expectations = [
        new_hmm.expected_states(data, input, mask, tag)
        for data, input, mask, tag in zip(datas, inputs, masks, tags)
    ]

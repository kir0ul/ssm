...
true_lds = ssm.LDS(
    ...
    emission_kwargs=dict(link="softplus", hidden_layer_sizes=(50, 50)),
)
true_lds.dynamics.As[0] = 0.95 * random_rotation(D, theta=(1) * np.pi / 20)
...
for i in range(splt):
    for j in range(splt):
        ...
        if n < N:
            ...
            im = plt.imshow(
                tuning_curves[:, n].reshape((npts, npts)),
                vmin=0,
                vmax=vmax,
                cmap="Greys",
            )

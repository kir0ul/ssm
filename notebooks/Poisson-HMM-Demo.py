...
plt.scatter(xlist, ylist, marker="o", s=10**3, edgecolors="black", facecolors="gray")
plt.xlim(-150, 150)
plt.ylim(-150, 150)

for i in range(num_targets):
    plt.annotate(
        str(i + 1), (xlist[i] - 5, ylist[i]), color="white", weight="bold", size=16
    )
...
init_dist = ssm.init_state_distns.FixedInitialStateDistribution(
    num_states, num_neurons, pi0=np.array([1, 0, 0, 0, 0])
)

...
true_slds = ssm.SLDS(
    emissions_dim, n_disc_states, latent_dim, emissions="gaussian_orthog"
)

for k in range(n_disc_states):
    true_slds.dynamics.As[k] = 0.95 * random_rotation(
        latent_dim, theta=(k + 1) * np.pi / 20
    )
...
for k in range(n_disc_states):
    ...
    if len(next_states) > 0 and len(curr_states) > 0:
        plt.plot(
            (curr_states[-1, 0], next_states[0, 0]),
...
q_mf_elbos, q_mf = slds.fit(
    data, method="bbvi", variational_posterior="mf", num_iters=1000
)
...
q_struct_elbos, q_struct = slds.fit(
    data, method="bbvi", variational_posterior="tridiag", num_iters=1000
)
...
for d, ax in enumerate(axs):
    ax.plot(
        states_x[:, d] + 4 * d, "-", color=colors[0], label="True" if d == 0 else None
    )
    ax.plot(
        q_lem_x[:, d] + 4 * d,
        "-",
...
elbos, posterior = slds.approximate_posterior(
    data,
    method="laplace_em",
    variational_posterior="structured_meanfield",
    num_iters=50,
...
plt.suptitle(
    "True and Inferred States using Structured Meanfield Posterior", va="baseline"
)
plt.tight_layout()
# -
...
for d, ax in enumerate(axs):
    ax.plot(
        data_x[:, d] + 4 * d, "-", color=colors[0], label="True" if d == 0 else None
    )
    ax.plot(
        x_est[:, d] + 4 * d,
        "-",

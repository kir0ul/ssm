...
import ssm
from ssm.variational import (
    SLDSMeanFieldVariationalPosterior,
...
from ssm.extensions.mp_srslds.emissions_ext import (
    GaussianOrthogonalCompoundEmissions,
    PoissonOrthogonalCompoundEmissions,
)
from ssm.extensions.mp_srslds.transitions_ext import (
    StickyRecurrentOnlyTransitions,
    StickyRecurrentTransitions,
)
...
alphas = (
    0.03 + 0.1 * np.random.rand(K)
)  # Determines the distribution of values in the dynamics matrix, for each discrete state
print("alphas:", alphas)

sparsity = 0.33  # Proportion of non-diagonal blocks in the dynamics matrix that are 0

e1 = 0.1  # Amount of noise in the dynamics
...
poiss_comp_emissions = PoissonOrthogonalCompoundEmissions(
    N=np.sum(N_vec), K=1, D=np.sum(D_vec), D_vec=D_vec, N_vec=N_vec, link="softplus"
)
...
v = 0.2 + 0.2 * np.random.rand(1)
for k in range(K):
    inc = np.copy(k)
    true_slds.transitions.Rs[k, D_vec_cumsum[inc] : D_vec_cumsum[inc] + 1] = v
    true_slds.transitions.Ss[k, D_vec_cumsum[inc] : D_vec_cumsum[inc] + 1] = v - 0.1
...
for k in range(K):
    ...
    A_mask = (
        np.random.rand(num_gr, num_gr) > sparsity
    )  # Make some blocks of the dynamics matrix 0
    ...
    for i in range(num_gr):
        A0[
            D_vec_cumsum[i] : D_vec_cumsum[i + 1], D_vec_cumsum[i] : D_vec_cumsum[i + 1]
        ] = (
            2
            * A0[
                D_vec_cumsum[i] : D_vec_cumsum[i + 1],
                D_vec_cumsum[i] : D_vec_cumsum[i + 1],
            ]
        )
    ...
    A = A / (np.max(np.abs(np.linalg.eigvals(A))) + 0.01)  # .97
...
zs, xs, _ = true_slds.sample(
    t_end
)  # Sample discrete and continuous latents from model for simulation

# Get spike trains that have an average firing rate of 0.25 per bin
tmp = np.mean(
    relu(
        np.dot(true_slds.emissions.Cs[0], xs.T)
        + 0.1 * true_slds.emissions.ds[0][:, None]
    ).T
)
mult = 0.25 / tmp
lams = relu(
    mult * np.dot(true_slds.emissions.Cs[0], xs.T)
    + 0.1 * true_slds.emissions.ds[0][:, None]
).T
...
# vmin,vmax=[-1,1]
vmin, vmax = [-0.5, 0.5]  # zoom in to see colors more clearly


for k in range(K):
    plt.figure(figsize=(4, 4))
    plt.imshow(
        true_slds.dynamics.As[k],
        aspect="auto",
        interpolation="none",
        vmin=vmin,
        vmax=vmax,
...
for g in range(K):
    plt.plot(
        np.dot(
            xs[st_t:end_t, D_vec_cumsum[g] : D_vec_cumsum[g + 1]],
            true_slds.transitions.Rs[j, D_vec_cumsum[g] : D_vec_cumsum[g + 1]].T,
        )
    )
plt.xlim(0, dur)
...
for g in range(K):
    plt.plot(
        np.dot(
            xs[st_t:end_t, D_vec_cumsum[g] : D_vec_cumsum[g + 1]],
            true_slds.transitions.Ss[j, D_vec_cumsum[g] : D_vec_cumsum[g + 1]].T,
        )
    )
plt.xlim(0, dur)
...
sro_trans = StickyRecurrentOnlyTransitions(
    K=K, D=np.sum(D_vec), l2_penalty_similarity=10, l1_penalty=10
)
...
)  # Regularization on the dynamics matrix

q_elbos_ar, q_ar = rslds.fit(
    ys,
    method="laplace_em",
...
for g in range(num_gr):
    ...
    lr.fit(
        q_ar.mean_continuous_states[0][:, D_vec_cumsum[g] : D_vec_cumsum[g + 1]],
        xs[:, D_vec_cumsum[g] : D_vec_cumsum[g + 1]],
    )
    R[D_vec_cumsum[g] : D_vec_cumsum[g + 1], D_vec_cumsum[g] : D_vec_cumsum[g + 1]] = (
        lr.coef_
    )
...
for k in range(K):
    ...
    plt.imshow(
        R @ rslds.dynamics.As[k] @ np.linalg.inv(R),
        aspect="auto",
        interpolation="none",
        vmin=-0.5,
        vmax=0.5,
...
for g in range(K):
    plt.plot(
        np.dot(
            q_ar.mean_continuous_states[0][
                st_t:end_t, D_vec_cumsum[g] : D_vec_cumsum[g + 1]
            ],
            rslds.transitions.Rs[j, D_vec_cumsum[g] : D_vec_cumsum[g + 1]].T,
        )
    )
plt.xlim(0, dur)
# plt.ylabel('Contribution towards \n switching to \n purple state',rotation=60)
...
for g in range(K):
    plt.plot(
        np.dot(
            ...
            rslds.transitions.Ss[j, D_vec_cumsum[g] : D_vec_cumsum[g + 1]].T,
        )
    )
plt.xlim(0, dur)
...
plt.plot(ys[st_t:end_t, nrn], alpha=0.5)  # true spiking activity

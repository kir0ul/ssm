...
def time_laplace_em_end2end(ncalls=5):
    ...
    for N, D, T, num_datas in params_list:
        ...
        total = timeit.timeit(
            lambda: lds_new.fit(datas, initialize=False, num_iters=1), number=ncalls
        )
...
def time_lds_sample(ncalls=20):
    ...
    for N, D, T, num_datas in params_list:
        ...
        _, posterior = lds_true.fit(
            datas, initialize=False, num_iters=1, method="laplace_em"
        )

        # Now we test the speed of sampling from this object.
        print("N, D, T, num_datas: = ", N, D, T, num_datas)
        total = timeit.timeit(
            lambda: posterior.sample_continuous_states(), number=ncalls
        )
        print("Avg time per call: %f" % (total / ncalls))

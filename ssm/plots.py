...
def plot_dynamics_2d(
    ...
    **kwargs,
    ...
    """
    assert dynamics_matrix.shape == (2, 2), (
        "Must pass a 2 x 2 dynamics matrix to visualize."
    )
    assert len(bias_vector) == 2, "Bias vector must have length 2."

    x_grid, y_grid = np.meshgrid(
        np.linspace(mins[0], maxs[0], npts), np.linspace(mins[1], maxs[1], npts)
    )

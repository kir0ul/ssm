...
class SparseAutoRegressiveObservations(AutoRegressiveObservations):
    ...
    def __init__(
        ...
        super(SparseAutoRegressiveObservations, self).__init__(
            K,
            D,
            M,
            lags=lags,
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        ll_init = np.column_stack(
            [
                stats.diagonal_gaussian_logpdf(data[:L], mu[:L], sigmasq * np.ones(D))
    ...
    def fit_sparse_linear_regression(self, ExxT, ExyT, EyyT, En, sigmasq, J0, h0, rho):
        ...
        for bo in range(B_out):
            ...
            assert B_in <= 16, (
                "You probably don't want to explicitly enumerate 2^B_{in}"
    ...
    def m_step(
        ...
        **kwargs,
        ...
        if self.iter_count > self.start_iter:
            ...
            h0_tmp = np.concatenate(
                (1e-8 * np.eye(D), np.zeros((D * (lags - 1), D)), np.zeros((M + 1, D)))
            )
            ...
            for k in range(K):
                ...
                A = Wk[:, :D]
                mask = 1 - np.kron(
                    np.identity(B_out), np.ones((S_out, S_out), dtype=bool)
                )  # NEED TO UPDATE W/ EITHER IN OR OUT
            ...
            for k in range(K):
                (
                    ...
                    self.As_mask[k],
                    self.sigmasqs[k],
                    self.As_mask_posteriors[k],
                ) = self.fit_sparse_linear_regression(
                    ExuxuTs[k],
                    ExuyTs[k],
                    EyyTs[k],
                    Ens[k],
...
class IdentityAutoRegressiveObservations(Observations):
    def __init__(self, K, D, M):
        ...
        self.log_sigma_init = np.zeros(
            D
        )  # log of the variance of the initial data point
        self.K = K
        self.D = D
    ...
    def log_likelihoods(self, data, input, mask, tag):
        ...
        ll1 = -0.5 * np.sum(
            (
                np.log(2 * np.pi * sigmas)
                + (data[1:, None, :] - data[:-1, None, :]) ** 2 / sigmas
            )
            * mask[1:, None, :],
            axis=2,
        )

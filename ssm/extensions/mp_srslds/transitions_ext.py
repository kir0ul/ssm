...
class StickyRecurrentTransitions(InputDrivenTransitions):
    ...
    def __init__(self, K, D, M=0, alpha=1, kappa=0, l2_penalty=1e-8, l1_penalty=1e-8):
        super(StickyRecurrentTransitions, self).__init__(
            K, D, M, alpha=alpha, kappa=kappa
        )
    ...
    def log_transition_matrices(self, data, input, mask, tag):
        ...
        log_Ps = (
            log_Ps + log_Ps_diag * mult_diag
        )  # Diagonal elements (stickness) from past observations
        log_Ps = (
            log_Ps + log_Ps_offdiag * mult_offdiag
        )  # Off diagonal elements (state switching) from past observations
...
class StickyRecurrentOnlyTransitions(Transitions):
    ...
    def __init__(self, K, D, M=0, l2_penalty_similarity=1e-8, l1_penalty=1e-8):
        ...
        self.Ws = npr.randn(
            K, M
        )  ## NOTE - W (REFLECTING INPUTS) HAS NOT BEEN USED HERE
        self.Rs = np.zeros([K, D])
        self.Ss = np.zeros([K, D])
        self.r = np.zeros(K)
        self.s = np.zeros(K)
    ...
    def log_transition_matrices(self, data, input, mask, tag):
        ...
        log_Ps = (
            log_Ps_diag * mult_diag
        )  # Diagonal elements (stickness) from past observations
        log_Ps = (
            log_Ps + np.identity(self.K) * self.s
        )  # Diagonal elements (stickness) bias
        log_Ps = (
            log_Ps + log_Ps_offdiag * mult_offdiag
        )  # Off diagonal elements (state switching) from past observations
        log_Ps = (
            log_Ps + (1 - np.identity(self.K)) * self.r
        )  # Off diagonal elements (state switching) bias
    ...
    def neg_hessian_expected_log_trans_prob(
        ...
        for k1 in range(self.K):
            for k2 in range(self.K):
                vtilde = vtildes[:, k1, k2][
                    :, None
                ]  # There's a chance the order of k1 and k2 is flipped
                if k1 == k2:
                    Rv = vtilde @ self.Ss[k2 : k2 + 1, :]
                    hess += Ez[k1, k2] * (
                        np.einsum(

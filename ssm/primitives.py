...
from ssm.cstats import (
    _blocks_to_bands_lower,
    _blocks_to_bands_upper,
    _bands_to_blocks_lower,
    _bands_to_blocks_upper,
    _transpose_banded,
    vjp_cholesky_banded_lower,
    _vjp_solve_banded_A,
    _vjp_solveh_banded_A,
)
...
def bands_to_blocks(A_banded, lower=True):
    ...
    return (
        _bands_to_blocks_lower(A_banded) if lower else _bands_to_blocks_upper(A_banded)
    )
...
def grad_cholesky_banded(L_banded, A_banded, lower=True):
    assert lower, (
        "Only implemented lower form so far. Need to do some \
...
def convert_lds_to_block_tridiag(As, bs, Qi_sqrts, ms, Ri_sqrts):
    ...
    J_diag = (
        np.concatenate(
            [-np.matmul(np.swapaxes(As, -1, -2), J_lower_diag), np.zeros((1, D, D))]
        )
    ...
    h = (
        np.concatenate(
            [np.matmul(J_lower_diag, bs[:, :, None])[:, :, 0], np.zeros((1, D))]
        )
...
def cholesky_lds(As, bs, Qi_sqrts, ms, Ri_sqrts):
    J_diag, J_lower_diag, _ = convert_lds_to_block_tridiag(
...
def lds_log_probability(x, As, bs, Qi_sqrts, ms, Ri_sqrts):
    ...
    return block_tridiagonal_log_probability(
        x, *convert_lds_to_block_tridiag(As, bs, Qi_sqrts, ms, Ri_sqrts)
    )
